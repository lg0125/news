...
import com.aliyuncs.green.extension.uploader.UploadCredentials;
...
public class ClientUploader {
    ...
    private UploadCredentials getCredentialsFromServer() throws Exception{
        ...
        try{
            ...
            if (httpResponse.isSuccess()) {
                ...
                if (200 == scrResponse.getInteger("code")) {
                    ...
                    return new UploadCredentials(
                            data.getString("accessKeyId"),
                            data.getString("accessKeySecret"),
                            data.getString("securityToken"),
                            data.getLongValue("expiredTime"),
                            data.getString("ossEndpoint"),
                            data.getString("ossInternalEndpoint"),
                            data.getString("uploadBucket")
                    );
                }

                String requestId = scrResponse.getString("requestId");
                throw new RuntimeException(
                        "get upload credential from server fail. requestId:" +
                                requestId +
                                ", code:" +
            ...
            throw new RuntimeException(
                    "get upload credential from server fail. http response status:" +
